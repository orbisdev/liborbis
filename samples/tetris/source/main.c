/*
 * liborbis tetris-c port
 * -----------------------
 *
 * a stripped port of code from https://github.com/epergo/tetris-C
 *
 * basically, SDL is not needed if we already have liborbis2d to do raster graphic,
 * or (at time of writing) SDL was not yet available, or major version was not met;
 *
 * 2019, masterzorag
 */
...
void updateController()
{
    ...
    if(ret==0)
    {
        ...
        if(orbisPadGetButtonPressed(ORBISPAD_DOWN) || orbisPadGetButtonHold(ORBISPAD_DOWN))
        {
            ...
            moveDown(8);  // speed_multiplicator
        ...
        moveDown(1);  // tetris default movedown piece!
        // take a breath, 10 microsecond
...
int main(int argc, char *argv[])
{
    ...
    Mod_Init(0);
    Mod_Load("host0:tetris.mod");
    Mod_Play();
    orbisAudioResume(0);

    init_tetris();  // setup tetris initial state
