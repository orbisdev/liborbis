...
{
    debugNetPrintf(DEBUG,"text_buffer_add_text(%p, %p, %p, %s)\n", self, pen, markup, text);
    size_t i;
    ...
    if( !markup->font )
    {
        debugNetPrintf(DEBUG, "Houston, we've got a problem !\n" );
        return;
    }
...
log: [PS4][DEBUG]: current: 2002ca3a0, previous: 0
*/
                      ...
                      const char * current, const char * previous )
{
    ...
    //debugNetPrintf(DEBUG,"current: %p, previous: %p\n", current, previous);
    size_t vcount = 0;
    size_t icount = 0;
...
//debugNetPrintf(DEBUG,"HERE 1\n"); // XXX

    glyph = texture_font_get_glyph( font, current );
    
//debugNetPrintf(DEBUG,"HERE 2\n");

    black = texture_font_get_glyph( font, NULL );

//debugNetPrintf(DEBUG,"HERE 3\n");
