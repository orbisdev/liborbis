        ...
        const void *memory_base, size_t memory_size)
{
    ...
    debugNetPrintf(INFO, "texture_font_new_from_memory ret:%p\n", self);
...
//debugNetPrintf(DEBUG,"texture_font_find_glyph\t%p '%.4d'\n", self, ucodepoint);

    for( i = 0; i < self->glyphs->size; ++i )
    {
        //debugNetPrintf(DEBUG,"i: %d size:%d\n", i, self->glyphs->size);
...
float tl = 0;  // we use from main(), as last composed text_length, in pixels
                         ...
                         const char * codepoint )
{
    ...
    if (glyph) {
        tl += glyph->advance_x; // measure text length for alignement
        FT_Done_Face(face);
        FT_Done_FreeType(library);
        return 1;
    }
...
texture_font_get_glyph( texture_font_t * self,
                        const char * codepoint )
{
 // debugNetPrintf(DEBUG,"texture_font_get_glyph %p '%p'\n", self, codepoint);
    texture_glyph_t *glyph;
...
//debugNetPrintf(DEBUG,"Glyph '%d' has not been already loaded\n", *codepoint);
