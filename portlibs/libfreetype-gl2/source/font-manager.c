                                ...
                                const float size )
{
    ...
    debugNetPrintf(INFO, "ttf at %p %ld\n", ttf, _orbisFile_lastopenFile_size );

    font = texture_font_new_from_memory( self->atlas, size, ttf, _orbisFile_lastopenFile_size);
//  font = texture_font_new_from_file  ( self->atlas, size, filename );
    ...
    debugNetPrintf(DEBUG,"Unable to load \"%s\" (size=%.1f)\n", filename, size );
    return 0;
}
                                   ...
                                   const int italic )
{
    ...
    else
    {
        ...
        if( !filename )
        {
            debugNetPrintf(DEBUG, "No \"%s (size=%.1f, bold=%d, italic=%d)\" font available.\n",
    ...
    filename = strdup( family );
    debugNetPrintf(DEBUG,"font_manager_get_from_filename \"%s\" (size=%.1f)\n", filename, size );
                              ...
                              const markup_t *markup )
{
    assert( self );
    assert( markup );
    debugNetPrintf(DEBUG,"font_manager_get_from_markup: %p, %s %.1f %d %d\n",
                          self,
