...
int rpc_process_pdu(struct rpc_context *rpc, char *buf, int size)
{
	...
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s after zdrmem_create \n",__FUNCTION__);

	if (rpc->is_udp == 0) {
		if (zdr_int(&zdr, &recordmarker) == 0) {
				debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s recordmarker \n",__FUNCTION__);
		...
		if (!(recordmarker&0x80000000)) {
				debugNetPrintf(3,"[LIBNFS] %s recormarker 8----\n",__FUNCTION__);
	...
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s after udp check \n",__FUNCTION__);
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s after udp check 1 \n",__FUNCTION__);
	...
	if (recordmarker != 0 && rpc->fragments != NULL) {
		...
		char *ptr;
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s before destroy\n",__FUNCTION__);

		zdr_destroy(&zdr);
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s after destroy\n",__FUNCTION__);

		for (fragment = rpc->fragments; fragment; fragment = fragment->next) {
			total += fragment->size;
				debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s fragments\n",__FUNCTION__);

                        if (total < fragment->size) {
                        		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s fragment large\n",__FUNCTION__);
	...
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s before malloc 1\n",__FUNCTION__);
	...
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s after fragment check \n",__FUNCTION__);

        if (rpc->is_server_context) {
                int ret;
				debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s is_server_context \n",__FUNCTION__);
                ...
                debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s is_server_context return\n",__FUNCTION__);
                return ret;
        }

	pos = zdr_getpos(&zdr);
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s after zdr_getpos \n",__FUNCTION__);

	if (zdr_int(&zdr, (int *)&xid) == 0) {
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s xid err \n",__FUNCTION__);
		...
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s error xid return \n",__FUNCTION__);

		return -1;
	}
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s after zdr_setpos \n",__FUNCTION__);
	...
	q = &rpc->waitpdu[hash];
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s before for \n",__FUNCTION__);
	...
	for (pdu=q->head; pdu; pdu=pdu->next) {
		if (pdu->xid != xid) {
			prev_pdu = pdu;
			debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s for continue \n",__FUNCTION__);
			continue;
		}
		...
		if (rpc_process_reply(rpc, pdu, &zdr) != 0) {
				debugNetPrintf(2,"[LIBNFS] %s error reply in for \n",__FUNCTION__);
		...
		debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s end in for \n",__FUNCTION__);

		return 0;
	}
	debugNetPrintf(DEBUGNETLEVEL,"[LIBNFS] %s after for \n",__FUNCTION__);
